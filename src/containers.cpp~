...
void container::dimPrint(map<int8_t, string> symbols, vector<vector<int> > checkDims)
{
	for (unsigned long i=0; i<monoPows.size(); i++)
	{
		if (monoPows[i].num!=0)
	...
	for (int j=0; j<checkDims.size(); j++)
	{
		int newDim=0;
		for (int k=0; k<Powers[0].size(); k++)
		{
			newDim += checkDims[j][Powers[0][k].var]*Powers[0][k].pow;
		}
		firstDims.push_back(newDim);
	}
	
	bool addPiFac = false;
	bool printedPiFac = false;
	...
	for (unsigned long i=0; i<Coefficients.size(); i++)
	{
		if (addPiFac == false)
		{
			...
			{
				int newDim=0;
				for (int k=0; k<Powers[i].size(); k++)
				{
					newDim += checkDims[j][Powers[i][k].var]*Powers[i][k].pow;
				}
				if (newDim != firstDims[j]){addPiFac=true;}
			}
		}
